...
long	date_julian(long d, long m, long y)
{
	...
	if	( y < 0 )	return j;
	if	( m < 1 || m > 12 )	return j;
	if	( d < 1 || d > 31 ) return j;
	t = (m - 14L) / 12L;
...
int		date_weekday(long d, long m, long y)
{
	...
	return ((1461*(y+4800+(m-14)/12)/4+367*(m-2-12*((m-14)/12))/12-3*((y+4900+(m-14)/12)/100)/4+d)%7);
}
...
double	cmd_math1(long funcCode, var_t *arg)
{
	...
	case kwRND:
		...
		break;
	default:
		rt_raise("Unsupported buildin function call %ld, please report this bug (2)", funcCode);
		};
...
//		r = 1; // fake! but the GetTickCount() returns the number of ms from the start
	...
	case kwFRE:
		...
		break;
	default:
		rt_raise("Unsupported buildin function call %ld, please report this bug (3)", funcCode);
		};
...
void	cmd_ns1(long funcCode, var_t *arg, var_t *r)
{
	...
	case kwISLINK:
		...
		break;
	default:
		rt_raise("Unsupported buildin function call %ld, please report this bug (4)", funcCode);
		};
}
...
void	cmd_str1(long funcCode, var_t *arg, var_t *r)
{
	...
	case	kwRTRIM:
		...
		if	( *p != '\0' )	{	// ndc: 20/2/2001
			while ( *p )	p ++;
			p--;
			while ( p >= arg->v.p.ptr && (is_wspace(*p)) )	p --;
			p++;
			*p = '\0';
			}
		r->v.p.ptr = (byte *) tmp_alloc(strlen((char *) arg->v.p.ptr)+1);
	...
	case	kwENVIRONF:
		...
		break;
	default:
		rt_raise("Unsupported buildin function call %ld, please report this bug (5)", funcCode);
		};
}
...
void	cmd_str0(long funcCode, var_t *r)
{
	...
	case kwINKEY:
		...
		if	( dev_kbhit() )	{
			ch = dev_getch();	// MultiByte - dev_getchr() must return the extended code (2 bytes char)
	...
	case kwDATE:
		...
		r->v.p.size = 32;
		StrPrintF(r->v.p.ptr, "%02d/%02d/%04d", cur_date.day, cur_date.month, cur_date.year);
		#elif defined(_VTOS)
		RtcGetTime(&now);
		r->type = V_STR;
		r->v.p.ptr = tmp_alloc(32);
	...
	case kwTIME:
		...
		StrPrintF(r->v.p.ptr, "%02d:%02d:%02d", cur_date.hour, cur_date.minute, cur_date.second);
		#elif defined(_VTOS)
		RtcGetTime(&now);
		r->type = V_STR;
		r->v.p.ptr = tmp_alloc(32);
		...
		break;
	default:
		rt_raise("Unsupported buildin function call %ld, please report this bug (6)", funcCode);
		}
}
...
//	r->type = V_STR;
	...
	case kwLEFTOFLAST:
		...
		if	( !prog_error )	{
			...
			l2 = strlen(s2);
			p = NULL;
			while ( (lp = strstr(lp, s2)) != NULL )	{ p = lp; lp += l2; };
	...
	case kwMID:
		...
		break;
	default:
		rt_raise("Unsupported buildin function call %ld, please report this bug (7)", funcCode);
		}
...
void	cmd_numN(long funcCode, var_t *r)
{
	...
	case kwROUND:
		...
		break;
	default:
		rt_raise("Unsupported buildin function call %ld, please report this bug (10)", funcCode);
		}
}
...
void	cmd_genfunc(long funcCode, var_t *r)
{
	...
	case	kwIFF:
		...
		if	( !prog_error )	{
			...
			if  ( prog_error )	break;
			if	( !ch )	{
				eval(&arg);	// false-value (there is no jump-optimization, so we must execute that)
				if	( !prog_error )	{
	...
	case	kwJULIAN:
		{
			...
			if	( arg.type == V_STR )	{
				...
				m = par_getint();	if	( prog_error ) return;
				par_getcomma();		if	( prog_error ) return;
	...
	case	kwWDAY:
		{
			...
			if	( arg2.type == V_STR )	{
				...
				v_free(&arg2);
				if	( code_peek() == kwTYPE_SEP )	{
					par_getcomma();		if	( prog_error ) { v_free(&arg); return; }
					m = par_getint();	if	( prog_error ) { v_free(&arg); return; }
					par_getcomma();		if	( prog_error ) { v_free(&arg); return; }
					y = par_getint();	if	( prog_error ) { v_free(&arg); return; }
					}
				else	{
					// julian
	...
	case	kwPTSIGN:
		{
			...
			B = par_getpt();		if	( prog_error )	return;
			par_getcomma();			if	( prog_error )	return;
			Q = par_getpt();		if	( prog_error )	return;
	...
	case	kwSEQ:
		...
		break;
	default:
		rt_raise("Unsupported buildin function call %ld, please report this bug (11)", funcCode);
		};
}
