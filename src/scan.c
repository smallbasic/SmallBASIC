...
char *comp_getlist_insep(char *source, char_p_t * args, char *sep,
                         char *delims, int maxarg, int *count) SEC(BCSC2);
...
int comp_add_external_proc(const char *proc_name, int lib_id)
{
  ...
  if (comp_extproctable == NULL) {
    comp_extprocsize = 16;
    comp_extproctable =
      (ext_proc_node_t *) tmp_alloc(sizeof(ext_proc_node_t) * comp_extprocsize);
  }
...
int comp_add_external_func(const char *func_name, int lib_id)
{
  ...
  if (comp_extfunctable == NULL) {
    comp_extfuncsize = 16;
    comp_extfunctable =
      (ext_func_node_t *) tmp_alloc(sizeof(ext_func_node_t) * comp_extfuncsize);
  }
...
int comp_get_uds_field_id(const char *field_name, int len)
{
  ...
  for (i = 0; i < comp_udscount; i++) {
    dbt_read(comp_udstable, i, &uds, sizeof(comp_struct_t));
    if (uds.field_id != -1 &&
        strlen(uds.name) == len && strncasecmp(uds.name, field_name, len) == 0) {
      return uds.field_id;
    }
  }
          ...
          if (idx != -1) {
            ...
            if (idx != -1) {
              bc_add_extfcode(&bc, comp_extfunctable[idx].lib_id,
                              comp_extfunctable[idx].symbol_index);
            }
            else {
...
void comp_text_line(char *text)
{
  ...
  if ((comp_bc_parm[0] == '=' || ladd || linc || ldec || leqop)
      && (idx != -1)) {
  ...
  if ((idx == kwCONST) ||
    ...
    case kwCASE:
      ...
      if (!comp_bc_parm || !comp_bc_parm[0] ||
...
mem_t comp_create_bin()
{
  ...
  hdr.size = sizeof(bc_head_t)
    ...
    + sizeof(bc_lib_rec_t) * comp_libcount + sizeof(bc_symbol_rec_t) * comp_impcount;
