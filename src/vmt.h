...
*/
dbt_t	dbt_create(const char *fileName, int flags)				SEC(PALMFS);
...
*/
void	dbt_close(dbt_t f)										SEC(PALMFS);
...
*/
void	dbt_write(dbt_t f, int index, void *ptr, int size)		SEC(PALMFS);
...
*	@param size is the size of the data
*/
void	dbt_read(dbt_t f, int index, void *ptr, int size)		SEC(PALMFS);
...
*/
int		dbt_recsize(dbt_t f, int index)							SEC(PALMFS);
...
*/
void	dbt_prealloc(dbt_t f, int num, int size)				SEC(PALMFS);
...
*/
int		dbt_count(dbt_t f)										SEC(PALMFS);
...
*	@param index is the index of the element
*/
void	dbt_remove(dbt_t f, int index)							SEC(PALMFS);
...
*	@return always 0
*/
int		dbt_setvar(dbt_t f, const char *varname, const char *varvalue)	SEC(PALMFS);
...
*/
char 	*dbt_getvar(dbt_t f, const char *varname)	SEC(PALMFS);
