...
void	chart_draw(int x1, int y1, int x2, int y2, double *vals, int count, double *xvals, int xcount, int chart, int marks) SEC(BIO2);
...
// third class
void	cmd_strN(long,var_t*)			SEC(BMATH);		// its ok, i want all functions to BMATH
...
double		line_segangle(int type, double Adx, double Ady, double Bdx, double Bdy) SEC(BMATH);
