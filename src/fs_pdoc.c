...
void	pdoc_loadpage(pdoc_t *doc, int index)
{
	...
	lseek(doc->handle, PDB_DATABASE_HDR_SIZE + PDB_RECORD_ENTRY_SIZE * index, SEEK_SET);
	read(doc->handle, &offset, 4);  	offset = BS32(offset);

	if ( index < doc->info.page_count ) {
		lseek(doc->handle, PDB_DATABASE_HDR_SIZE + PDB_RECORD_ENTRY_SIZE * (index+1), SEEK_SET);
		read(doc->handle, &next_offset, 4);  	next_offset = BS32(next_offset);
...
//	int			num_records;
	...
	if	( (strncmp(header.type, SID_TEXT, 4) != 0) || (strncmp(header.creator, SID_READ, 4) != 0) ) {
		close(h);
		return -5;
		}

//	num_records = BS32(header.record_list.num_records) - 1;
	lseek(h, PDB_DATABASE_HDR_SIZE + PDB_RECORD_ENTRY_SIZE * 0, SEEK_SET);	// RECORD 0
	read(h, &offset, 4);  	offset = BS32(offset);
...
int		pdoc_create_pdoc_from_file(const char *textfile, const char *pdocfile)
{
	...
	h_in = FileOpen(0, (char *) textfile, ID_UFST, ID_SmBa, fileModeAnyTypeCreator | fileModeReadOnly, &last_error);
	...
	for ( rec_num = 1; rec_num <= pages; rec_num ++ ) {
		...
		lseek(doc.handle, PDB_DATABASE_HDR_SIZE + PDB_RECORD_ENTRY_SIZE * rec_num, SEEK_SET);
		offset = BS32(offset);
		write(doc.handle, &offset, 4);
		offset = BS32(offset);
...
int		pdoc_open(dev_file_t *f)
{
	...
	if	( (DmFindDatabase(0, (char *) f->name) == 0) || (f->open_flags == DEV_FILE_OUTPUT) )	
		pdoc_create_empty(f->name);
	#endif
...
int		pdoc_close(dev_file_t *f)
{
	...
	stream_close(f);
	if	( (f->drv_dw[0] & DEV_FILE_OUTPUT) || (f->drv_dw[0] & DEV_FILE_APPEND) || (f->drv_dw[0] == 0) )	
