...
void	exec_usefunc2(var_t *var1, var_t *var2, addr_t ip)
{
	...
	// run
	v_set(tvar[SYSVAR_X], var1);	v_free(var1);
	v_set(tvar[SYSVAR_Y], var2);	v_free(var2);
	code_jump(ip);
	eval(var1);

	// restore X,Y
	v_set(tvar[SYSVAR_X], old_x);	v_free(old_x);	tmp_free(old_x);
	v_set(tvar[SYSVAR_Y], old_y);	v_free(old_y);	tmp_free(old_y);
}
...
void	exec_usefunc3(var_t *var1, var_t *var2, var_t *var3, addr_t ip)
{
	...
	v_set(tvar[SYSVAR_X], var1);	v_free(var1);
	v_set(tvar[SYSVAR_Y], var2);	v_free(var2);
	v_set(tvar[SYSVAR_Z], var3);	v_free(var3);
	code_jump(ip);
	eval(var1);
	...
	v_set(tvar[SYSVAR_X], old_x);	v_free(old_x);	tmp_free(old_x);
	v_set(tvar[SYSVAR_Y], old_y);	v_free(old_y);	tmp_free(old_y);
	v_set(tvar[SYSVAR_Z], old_z);	v_free(old_z);	tmp_free(old_z);
}
...
void	par_getvar(var_t *var)
{
	...
	code = code_peek();
	switch ( code )	{
	case	kwTYPE_LINE: case kwTYPE_EOC: case	kwTYPE_SEP:
		err_syntax();
		return;
	default:
		eval(var);
...
int		par_getpoly(pt_t **poly_pp)
{
	...
	// zero-length array
	if	( var->v.a.size == 0 )	{
		if	( alloc )	{ v_free(var);	tmp_free(var);	}
		return 0;
		}
	...
	if	( rqcount > pcount )	
		...
		*/
		...
		while ( *fmt_p )	{
			...
			if ( *fmt_p >= 'a' && optcount && ( (curpar < pcount)? par_massget_type_check(*fmt_p, &ptable[curpar]) : 0) )	
