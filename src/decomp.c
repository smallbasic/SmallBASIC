...
void dump_bytecode(FILE * output)
{
  ...
  if (prog_symcount) {
    ...
    for (i = 0; i < prog_symcount; i++) {
      fprintf(output,
              ...
              (prog_symtable[i].type ==
               stt_variable) ? "variable" : ((prog_symtable[i].type ==
  ...
  if (prog_expcount) {
    ...
    for (i = 0; i < prog_expcount; i++) {
      fprintf(output,
              ...
              (prog_exptable[i].type ==
               stt_variable) ? "variable" : ((prog_exptable[i].type ==
                                              stt_procedure) ? "procedure" :
  ...
  do {
    ...
    fprintf(output, "  %08d: %c%c %c; ",
            (int)prog_ip - 1,
            "0123456789ABCDEF"[h], "0123456789ABCDEF"[l], (b >= 32) ? b : 42);
