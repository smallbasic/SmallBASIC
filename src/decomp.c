...
void	dump_bytecode(FILE *output)
{
	...
	if	( prog_libcount )	{
	    ...
	    for ( i = 0; i < prog_libcount; i ++ )	{
		    fprintf(output, 
				...
				((prog_libtable[i].type == 1) ? prog_libtable[i].id | UID_UNIT_BIT: prog_libtable[i].id)
				);
			}
		}
	else
	...
	if	( prog_symcount )	{
	    ...
	    for ( i = 0; i < prog_symcount; i ++ )	{
		    fprintf(output, 
				"  %s: type %s, lid 0x%0X, sid %d, vid %d - tid %d, eidx %d\n", 
				prog_symtable[i].symbol, 
				(prog_symtable[i].type == stt_variable) ? "variable" : ((prog_symtable[i].type == stt_procedure) ? "procedure" : "function"),
				prog_symtable[i].lib_id,
				prog_symtable[i].sym_id, 
				prog_symtable[i].var_id,
				prog_symtable[i].task_id,
				prog_symtable[i].exp_idx
				);
			}
		}
	else
	...
	if	( prog_expcount )	{
	    ...
	    for ( i = 0; i < prog_expcount; i ++ )	{
		    fprintf(output, 
				"  %s: type %s, address %d, related variable id %d\n", 
				prog_exptable[i].symbol, 
				(prog_exptable[i].type == stt_variable) ? "variable" : ((prog_exptable[i].type == stt_procedure) ? "procedure" : "function"),
				prog_exptable[i].address, prog_exptable[i].vid);
			}
		}
	else
	...
	do	{
		...
		default:
			...
			case	kwENDIF:	case	kwNEXT:		case	kwWEND:		case	kwUNTIL:
				fprintf(output, "address1 (true?) %d, address2 (false?) %d ", code_getaddr(), code_getaddr());
				break;
				}
			}
