...
//			flags |= 1;	// there is no memory, so, create it to "disk"
		...
		// ndc 16/9/2002:
		// It is crazy! the new VMT (file-streams) is faster than standard mem_alloc/mem_free!!!
		// However, it is need time to allocate/free the space in the file 
		// (so, that is why the delay before and after compilation)
		// I am suspecting that is depended on emulator (on real Palm the results may be different)
//		}
	#endif
...
void	dbt_write(dbt_t t, int index, void *ptr, int size)
{
	if	( vmt[t].flags & 1 )	{
		...
		if	( vmt[t].size <= index )	{
			mem_unlock(vmt[t].m_handle);
			vmt[t].m_handle = mem_realloc(vmt[t].m_handle, sizeof(mem_t) * (vmt[t].size + MEM_VMT_GROWSIZE));
		...
		else if ( size != MemHandleSize(vmt[t].m_table[index]) )	{	// resize record
