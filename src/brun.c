 ...
 */
void
    code_pop_and_free(stknode_t * node)
{
#if defined(_UnixOS) && defined(_CHECK_STACK)
    int i;
#endif
    ...
    if (prog_stack_count) {
        ...
        case kwBYREF:
            ...
            break;
        case kwTYPE_VAR:
            if ((cur_node->x.param.vcheck == 1) ||
                (cur_node->x.param.vcheck == 0x81)) {
    ...
    if (code_peek() != kwTYPE_LEVEL_BEGIN) {
        ...
        if (!prog_error) {
            if ((int) array_index < basevar_p->v.a.size &&
                (int) array_index >= 0) {
                var_p =
                    (var_t *) (basevar_p->v.a.ptr +
                               (array_index * sizeof(var_t)));
...
int brun_create_task(const char *filename, mem_t preloaded_bc, int libf)
{
    ...
    if (memcmp(source, "SBUn", 4) == 0) {       // load a unit
        ...
        if (prog_expcount) {
            prog_exptable =
                (unit_sym_t *) tmp_alloc(prog_expcount * sizeof(unit_sym_t));
    ...
    if (prog_libcount) {
        prog_libtable =
            (bc_lib_rec_t *) tmp_alloc(prog_libcount * sizeof(bc_lib_rec_t));
    ...
    if (prog_symcount) {
        prog_symtable =
            (bc_symbol_rec_t *) tmp_alloc(prog_symcount *
                                          sizeof(bc_symbol_rec_t));
        ...
        for (i = 0; i < prog_libcount; i++) {
            if (prog_libtable[i].type == 1) {
                ...
                for (j = 0; j < prog_symcount; j++) {
                    ...
                    if ((prog_symtable[j].lib_id & (~UID_UNIT_BIT)) ==
                        prog_libtable[i].id) {

                        // find symbol by name (for sure) and update it
                        // this is required because lib may be newer than
                        // parent
                        for (k = 0; k < taskinfo(lib_tid)->sbe.exec.expcount;
                             k++) {
                            if (strcmp
                                (pname,
                                 taskinfo(lib_tid)->sbe.exec.exptable[k].
                                 symbol) == 0) {
...
#  endif
        ...
        if (fp) {
            fprintf(fp, "Error: %d\nLine: %d\nFile: %s\nMessage: %s\n",
                    gsb_last_error, gsb_last_line, gsb_last_file,
                    gsb_last_errmsg);
            fclose(fp);
        }
    }
