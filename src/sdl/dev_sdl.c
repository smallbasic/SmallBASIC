...
static int mouse_mode, mouse_x, mouse_y, mouse_b, mouse_upd, mouse_down_x, mouse_down_y,
    mouse_pc_x, mouse_pc_y;
...
struct voice_info {
    int period;                 // we don't need float because we could calculate just integer 
                                // number of sample
...
void audio_callback(void *user, unsigned char *stream, int length)
{
    ...
    if (audio_head != audio_tail || audio_info[audio_head].remain_ms) {
        ...
        volume = SDL_MIX_MAXVOLUME;     // we implemented the volume handling in software
                                        // because
        // don't know hardware mixer device is working or not!
        sound_len_us = info->remain_ms * 1000;  // get the length of the sound / silent in
                                                // microsec
        ...
        do {
            if (info->period >= 1) {    // we have some sound
                ...
                left -= count;
                info->toggle -= count;
                sound_len_us -= count * one_sample_len_us;      // yes theoretically the sound 
                                                                // length can be 
        ...
        if (left <= 0 && sound_len_us > 0) {    // we have remain some portion of the sound /
                                                // silent!
            info->remain_ms = sound_len_us / 1000;      // take back to que
        }

        if (mix)
            SDL_MixAudio(stream, (unsigned char *)mixbuf, length, volume);      // and do the
                                                                                // task - play 
                                                                                // 
    }
}
...
void osd_sound(int freq, int ms, int vol, int bgplay)
{
    ...
    if (has_audio) {
        ...
        loops = (int)ms / lchunk;       // the longness is rounded to chunk size!! cc. 100
                                        // msec!
        ...
        if (vol < 0)
            vol = 0;
        if (vol > 100)
            vol = 100;          // volume should be between 0 and 100 according to spec!
        info->vol = vol;
        // the correct info->setting is according to the volume because we don't know hardware 
        // mixer is working or not!
        info->setting = ((audiospec.format & 0xff) == 0x8) ? 0x7f : 0x7fff;     // 8 or 16 bit 
                                                                                // sample
                                                                                // size?
        ...
        if ((freq < audiospec.freq / 2) && (freq != 0)) {
            ...
            info->period = 0;   // we generate silent!! for example sound 32767 definitely
                                // larger then 22050!
        info->toggle = info->period;
        ...
        if (loops)
            for (i = 1; i <= loops; i++) {      // handling consecutive chunks - long sound
                ...
                // next line try to compensate the noise if the end of the previous chunk are
        ...
        if (last_loop)          // handling the last chunk from a long sound or a quick one
        {
            ...
            // next line try to compensate the noise if the end of a chunk are in a negativ
            // half period
            // if no any loops (loops is zero) this does nothing might be the wave start with
            // a negative half
            if (info->period > 1)
...
void dev_image(int handle, int index, int x, int y, int sx, int sy, int w, int h)
{
    ...
    if (filep->open_flags == DEV_FILE_INPUT) {
        if (!(img = Look_in_cache(filep->name))) {
            ...
            img = SDL_DisplayFormat(img1);      // convert the image to the same format of
        ...
        if (img1 != NULL) {
            ...
            SDL_BlitSurface(img1, NULL, img, NULL);     // copy the whole original image to
                                                        // 0,0
            SDL_FreeSurface(img1);
        } else
        ...
        dev_fclose(handle);     // We have to close the file because SDL_SaveBMP do not use a
                                // shared open!
        SDL_SaveBMP(img, BMPfilename);
        SDL_FreeSurface(img);
        dev_fopen(handle, (char *)&BMPfilename, DEV_FILE_APPEND);       // reopen the file
                                                                        // with APPEND!!!
    }
}
