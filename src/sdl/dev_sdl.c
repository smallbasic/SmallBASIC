 ...
 *
 *           add image support (need a new compiler switch -DHAVE_SDL_IMAGE and a small modification in device.c!)
 *               with SDL, output graphic file format is BMP because SDL support just that one but for write
 *               with support of SDL_image library the input can be JPG, GIF, BMP, PNG, TIFF
 *
 *           image support has a cache parameters:
 *               MAX_IMAGE_IN_CACHE - maximum how many image could be in the cache independently from size
 ...
 *               for example esc, cr, lf could have graphical form (except the null which terminate the string)
 *
...
static int font_l = 1;      // this is the length of a row in bytes equal INT((font_w + 7) / 8)
...
struct voice_info {
    int      period;         // we don't need float because we could calculate just integer number of sample
...
void   audio_callback(void *user, unsigned char* stream, int length)
{
    ...
    if ( audio_head != audio_tail || audio_info[audio_head].remain_ms )   {
        ...
        volume = SDL_MIX_MAXVOLUME;     // we implemented the volume handling in software because
                                        // don't know hardware mixer device is working or not!
        sound_len_us = info->remain_ms * 1000;  // get the length of the sound / silent in microsec
        one_sample_len_us = 1000 * 1000 / audiospec.freq ; // one sample length in microsec (cc. 23 microsec)
        ...
        do {
            if ( info->period >= 1 ) {      // we have some sound
                ...
                left -= count;
                info->toggle -= count;
                sound_len_us -= count * one_sample_len_us;  // yes theoretically the sound length can be 
                                                            // quicker than half period but it is unrealistic!
                //                while (count--) mixbuf[j++] = audiospec.silence + info->setting; 
                memset(&mixbuf[j], audiospec.silence + info->setting, count); j+=count; count=0;
            ...
            } else {                                    // we have no sound but calculate length of silence
                ...
                if (count < left) {
                    ...
                    sound_len_us -= left * one_sample_len_us ;  // the remaining length in microsec
                    count = left;
                }
        ...
        if (left <= 0 && sound_len_us > 0) {        // we have remain some portion of the sound / silent!
            info->remain_ms = sound_len_us / 1000;  // take back to que
        }

        if ( mix )
            SDL_MixAudio(stream, (unsigned char *) mixbuf, length, volume); // and do the task - play 
    }
}
...
void make_update(int x, int y, int x1, int y1)
{
    ...
    if (y < y1)
        ...
        { ty = y1; ty1 = y; }
    if (tx < 0) tx = 0;
    if (tx1 > os_graf_mx-1) tx1=os_graf_mx-1;
    if (ty < 0) ty = 0;
    if (ty1 > os_graf_my-1) ty1=os_graf_my-1;

    // the rectangle in correct order is tx,ty,tx1,ty1
    if (Update_rect.x < 0) { Update_rect.x = tx; Update_rect.w = tx1-tx; }
    if (Update_rect.y < 0) { Update_rect.y = ty; Update_rect.h = ty1-ty; }
    if (Update_rect.x > tx) { Update_rect.w =  Update_rect.x + Update_rect.w - tx ; Update_rect.x = tx;}
    if (Update_rect.x + Update_rect.w < tx1) {Update_rect.w = tx1 - Update_rect.x ;}
    if (Update_rect.y > ty) { Update_rect.h = Update_rect.y + Update_rect.h - ty ; Update_rect.y = ty;}
    if (Update_rect.y + Update_rect.h < ty1) {Update_rect.h = ty1 - Update_rect.y;}
    scr_update = 1;
}
...
int      osd_devinit()
{
    ...
    int      i, w=640, h=480, d=16;

    cur_x = cur_y = 0;
    bytespp = 1;
    mouse_mode= mouse_x= mouse_y= mouse_b= mouse_upd= mouse_down_x= mouse_down_y= mouse_pc_x= mouse_pc_y= 0;
    ...
    font_w = 8;
    font_h = 16;
    font_l = 1;     // this is the length of a row in bytes equal INT((font_w+7)/8)
    ...
    if(getenv("LC_ALL") || getenv("LC_CTYPE") || getenv("LANG")) {
        ...
        if (!l) l = (char *)getenv("LC_CTYPE");
        if (!l) l = (char *)getenv("LANG");
        if (zstr(l, "en") || zstr(l, "fr") ||
            zstr(l, "de") || zstr(l, "dk") ||
            zstr(l, "it") || zstr(l, "es") ||
            zstr(l, "nl") || zstr(l, "ISO-8859-1")) 
            { savedsystemfont= currentsystemfont=0; currentfont= font8x16i1;}
        if (zstr(l, "pl") || zstr(l, "hr") ||
            zstr(l, "hu") || zstr(l, "cs") ||
            zstr(l, "ro") || zstr(l, "sl") ||
            zstr(l, "sk") || zstr(l, "ISO-8859-2")) 
            { savedsystemfont= currentsystemfont=1; currentfont= font8x16i2;}
        if (zstr(l, "ru")|| zstr(l, "uk") ||
            zstr(l, "bg") || zstr(l, "ISO-8859-5")) 
            { savedsystemfont= currentsystemfont=2; currentfont= font8x16i5;}
        if (zstr(l, "el") || zstr(l, "gr") || zstr(l, "ISO-8859-7")) 
            { savedsystemfont= currentsystemfont=3; currentfont= font8x16i7;}
    }

    /* setup video mode */
    screen = SDL_SetVideoMode(w, h, d, 0);
    if ( screen == NULL ) {
        fprintf(stderr, "SDL: Couldn't set %dx%dx%d video mode: %s\nUse SB_SDLMODE environment variable (set SB_SDLMODE=800x600x16)\n", w, h, d, SDL_GetError());
        return 0;
    }
    ...
    os_color = 1;
    if   ( os_color_depth != 8 && os_color_depth != 16 && os_color_depth != 24 && os_color_depth != 32 )   {
...

    {
        ...
        snprintf(cbuf, 256, "SmallBASIC/SDL %dx%dx%d - %s", w, h, d, g_file);
    ...
    if   ( has_audio )   {
        ...
        /* Set the audio format
           ...
           freq/2 = gives the highest frequency of generated tone 44100 means 22050 Hz is the highest
           format = the quality of the tone 8 bit or 16 bit
           channels = 1 mono, 2 stereo
           samples = number of samples in a chunk will limit the lowest tone frequency
           for example 4096 means 44100/4096 * 2 = 21 Hz is the lowest tone
           on the other hand that means on chunk is 4096/44100 *1000 = 92 msec long
           the required buffer size is samples * format&0xff/8 * channel - calculated by SDL
        */
        wanted.freq     = 44100;
    ...
    Init_cache();
#endif
    SDL_EnableKeyRepeat(SDL_DEFAULT_REPEAT_DELAY, SDL_DEFAULT_REPEAT_INTERVAL); // enable keyboard repeate
    /* Enable Unicode translation */
    SDL_EnableUNICODE( 1 );
    while ( dev_kbhit() );   // clear the keyboard que
...
int      osd_devrestore()
{
    ...
    if (!fast_quick) {      // if the user would like to see anything wait
        ...
        while ( !dev_kbhit() ) SDL_Delay(50);   // wait for key hit but not eat the cpu!
    }
    // if the user would like to quit w/o wait just set the same zero for fg and bg like COLOR 0,0
...
void   direct_setpixel(int x, int y, long c)
{
    ...
    if ( c == -1 ) return;                  // this case of transparent color
    if   ( x >= 0 && x < os_graf_mx )   {
        if   ( y >= 0 && y < os_graf_my )      {
...
void   direct_hline(int x, int x2, int y)
{
    ...
    if ( fg_screen_color == -1 ) return;                    // this case of transparent color
    if   ( x > x2 )   {
        i  = x;
        x  = x2;
        x2 = i;
...
void   osd_drawchar(int x, int y, byte ch, int overwrite, long fg_rgb, long bg_rgb)
{
    ...
    if (font_w > 8) {
        for ( i = 0; i < font_h; i ++, offset += font_l )   {
            ...
            switch (font_w) {
            case 16:
                if (data1 & (1<<7))      direct_setpixel(x+8, y+i, fg_rgb);
                else if ( overwrite )    direct_setpixel(x+8, y+i, bg_rgb);
                if (data1 & (1<<6))      direct_setpixel(x+9, y+i, fg_rgb);
                else if ( overwrite )    direct_setpixel(x+9, y+i, bg_rgb);
                if (data1 & (1<<5))      direct_setpixel(x+10, y+i, fg_rgb);
                else if ( overwrite )    direct_setpixel(x+10, y+i, bg_rgb);
                if (data1 & (1<<4))      direct_setpixel(x+11, y+i, fg_rgb);
                else if ( overwrite )    direct_setpixel(x+11, y+i, bg_rgb);
                if (data1 & (1<<3))      direct_setpixel(x+12, y+i, fg_rgb);
                else if ( overwrite )    direct_setpixel(x+12, y+i, bg_rgb);
                if (data1 & (1<<2))      direct_setpixel(x+13, y+i, fg_rgb);
                else if ( overwrite )    direct_setpixel(x+13, y+i, bg_rgb);
                if (data1 & (1<<1))      direct_setpixel(x+14, y+i, fg_rgb);
                else if ( overwrite )    direct_setpixel(x+14, y+i, bg_rgb);
                if (data1 & (1<<0))      direct_setpixel(x+15, y+i, fg_rgb);
                else if ( overwrite )    direct_setpixel(x+15, y+i, bg_rgb);
                break;
            case 15:
                if (data1 & (1<<7))      direct_setpixel(x+8, y+i, fg_rgb);
                else if ( overwrite )    direct_setpixel(x+8, y+i, bg_rgb);
    ...
    while ( *p )   {
        if (next_is_graphic) {
            ...
            if   ( !con_use_reverse )   {
                ...
                if   ( con_use_bold )   
                    osd_drawchar(cur_x-1, cur_y, *p, 0, get_screen_color(GET_FG_COLOR), get_screen_color(GET_BG_COLOR));
            }
            else   {
                osd_drawchar(cur_x, cur_y, *p, 1, get_screen_color(GET_BG_COLOR), get_screen_color(GET_FG_COLOR));
                if   ( con_use_bold )   
                    osd_drawchar(cur_x-1, cur_y, *p, 0, get_screen_color(GET_BG_COLOR), get_screen_color(GET_FG_COLOR));
            }
        ...
        case   SDL_KEYDOWN:
            ...
            if   ( (ch == SDLK_c && (ev.key.keysym.mod & KMOD_CTRL)) || ch == SDLK_BREAK )   {   // break
                ...
                if   ( ch > 0 && ch < 255 )   {
                    ...
                    upr   = ( ((ev.key.keysym.mod & (KMOD_CAPS))) && ((ev.key.keysym.mod & (KMOD_SHIFT)) == 0) )
                        ||
                        ( ((ev.key.keysym.mod & (KMOD_CAPS)) == 0) && ((ev.key.keysym.mod & KMOD_SHIFT)) );
                ...
                if (ch > 0 && ev.key.keysym.unicode)
                    dev_pushkey(ev.key.keysym.unicode & 0xFF);  // using kernel keyboard mapping
...
void   osd_sound(int freq, int ms, int vol, int bgplay)
{
    ...
    if   ( has_audio )   {
        ...
        // wrong!!      loops = ((ms * audiospec.freq / freq / 2.0) / audiospec.samples) / 1000;
        ...
        loops = (int) ms / lchunk ;             // the longness is rounded to chunk size!! cc. 100 msec!
        ...
        if (vol < 0) vol = 0;
        if (vol > 100) vol = 100;             // volume should be between 0 and 100 according to spec!
        info->vol = vol;
        // the correct info->setting is according to the volume because we don't know hardware mixer is working or not!
        info->setting = ( (audiospec.format & 0xff) == 0x8 ) ? 0x7f : 0x7fff ; // 8 or 16 bit sample size?
        info->setting = (info->setting * info->vol) / 100;        // take care on integer rounding!

        if ( (freq < audiospec.freq / 2) && (freq != 0) ) {
            info->period  = (int) (audiospec.freq / freq / 2.0);
        } 
        else    
            info->period  = 0;      // we generate silent!! for example sound 32767 definitely larger then 22050!
        info->toggle = info->period;

        // 

        if (loops)
            for ( i = 1; i <= loops; i ++ )   { // handling consecutive chunks - long sound
                audio_info[audio_tail] = *info;
                audio_info[audio_tail].remain_ms = lchunk ;
                // next line try to compensate the noise if the end of the previous chunk are in a negativ half period
                if (info->period > 1)
                    if (!((((i-1)*lchunk) / info->period) & 1)) audio_info[audio_tail].setting = -info->setting;
                audio_tail ++;
                if   ( audio_tail >= AUDIO_STACK_MAX )
                    audio_tail = 0;
            }
        if (last_loop)                // handling the last chunk from a long sound or a quick one
            {
                audio_info[audio_tail] = *info;
                audio_info[audio_tail].remain_ms = last_loop ;
                // next line try to compensate the noise if the end of a chunk are in a negativ half period
                // if no any loops (loops is zero) this does nothing might be the wave start with a negative half
                if (info->period > 1)
                    if (!(((loops*lchunk) / info->period) & 1)) audio_info[audio_tail].setting = -info->setting;
                audio_tail ++;
                if   ( audio_tail >= AUDIO_STACK_MAX )
                    audio_tail = 0;
            }
...
void dev_image(int handle, int index, int x, int y, 
    ...
    if (screen == NULL) return;     // SDL is not initialized
    ...
    if (filep->open_flags == DEV_FILE_INPUT) {
        if (!(img = Look_in_cache(filep->name))) {
            //          dev_fclose(handle);     
            // We have to close the file because SDL_LoadBMP do not use a shared open!
            //          img1 = SDL_LoadBMP((char *) &BMPfilename);
            img1 = IMG_Load_RW(SDL_RWFromFile((const char *) &BMPfilename, "rb"), 1); // SDL_image support!
            if (img1 == NULL) {
            ...
            img = SDL_DisplayFormat(img1);  // convert the image to the same format of screen for fast blitting
            SDL_FreeSurface(img1);
            //          dev_fopen(handle,(char *) &BMPfilename,saveflags);  // reopen the file - rewind
            if (img) Add_to_cache(img, filep->name);
        }
        if (img) {
            ...
            srect.y = sy;
            drect.x = x;
            drect.y = y;    // drect.w, .h ignored as input by BlitSurface but updated!
            if (index<0) {
            ...
            } else  // clear colorkeying even if it was declared in the image file
        ...
        if (img1 != NULL) {
            ...
            // create enough large surface to hold the image in the file and the saved one
            ...
            SDL_BlitSurface(img1,NULL,img,NULL);    // copy the whole original image to 0,0
            SDL_FreeSurface(img1);
        }
        else
            img = SDL_CreateRGBSurface(SDL_SWSURFACE, w, h, screen->format->BitsPerPixel, 
                                       screen->format->Rmask, screen->format->Gmask, 
                                       screen->format->Bmask, screen->format->Amask);
        srect.x = x; drect.x = sx;
        srect.y = y; drect.y = sy;
        srect.w = w; drect.w = w;
        srect.h = h; drect.h = h;
        SDL_BlitSurface(screen, &srect, img, &drect);           
        dev_fclose(handle);     // We have to close the file because SDL_SaveBMP do not use a shared open!
        SDL_SaveBMP(img,BMPfilename);
        SDL_FreeSurface(img);
        dev_fopen(handle,(char *) &BMPfilename,DEV_FILE_APPEND);    // reopen the file with APPEND!!!
    }
}
...
int dev_image_width(int handle, int index) {
    ...
    if (screen == NULL) return 0;       // SDL is not initialized
    if (filep)
        if (filep->open_flags == DEV_FILE_INPUT) {
            strcpy((char *) &BMPfilename,filep->name);
            savehandle = handle;
            ...
            if (!(img = Look_in_cache(filep->name))) {
                //          dev_fclose(handle);     
                // We have to close the file because SDL_SaveBMP do not use a shared open!
                //          img1 = SDL_LoadBMP((char *) &BMPfilename);
                img1 = IMG_Load_RW(SDL_RWFromFile((const char *)&BMPfilename, "rb"), 1);
                if (img1 == NULL) {
                    char buf[256];
