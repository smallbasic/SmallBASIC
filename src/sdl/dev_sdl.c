...
void	osd_bmpput(int x, int y, int write_mode, char *buf)
{
	...
	if	( write_mode )	{
		...
		bmp_get((bmp_header_t*) buf, x, y, x+bmp->dx, y+bmp->dy, bytespp * 8, osd_getpixel);
		bmp_combine((byte *) (newbmp+sizeof(bmp_header_t)), (byte*) (bmp+sizeof(bmp_header_t)), bytespp * bmp->dx, bmp->dy, write_mode);
