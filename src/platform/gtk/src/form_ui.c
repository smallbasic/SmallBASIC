...
void update_vars(GtkWidget *widget) {
  ...
  case ctrl_label:
    if (inf->var->type == V_STR && inf->var->v.p.ptr &&
        g_ascii_strcasecmp(inf->var->v.p.ptr, gtk_label_get_text(GTK_LABEL(widget))) != 0) {
...
void create_grid_row(var_t *row_p, GtkTreeStore *model, GtkTreeIter *parent_row, int n_columns) {
  GtkTreeIter row_iter;
  int col = 0;
  int i;

  if (row_p->type != V_ARRAY && row_p->type != V_STR && row_p->type != V_INT) {
    return;
  }
...
GtkWidget *create_grid(const char *caption, var_t *v) {
  ...
  g_free(types);
  gtk_tree_view_set_model(GTK_TREE_VIEW(view), gtk_tree_model_sort_new_with_model(GTK_TREE_MODEL(model)));
  g_object_unref(model);        // destroy model automatically with view

  GtkTreeSelection *selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(view));
  gtk_tree_selection_set_mode(selection, GTK_SELECTION_BROWSE);

  g_signal_connect((gpointer) view, "row_activated", G_CALLBACK(on_treeview_row_activated), NULL);
  ...
  if (sel_row->type == V_STR) {
    g_signal_connect(G_OBJECT(selection), "changed", G_CALLBACK(on_grid_selection), sel_row);
  ...
  if (-1 != par_massget("IIIIPSs", &x1, &x2, &y1, &y2, &v, &caption, &type)) {
    ...
    if (type) {
      if (g_ascii_strcasecmp("radio", type) == 0) {
        ...
        g_signal_connect((gpointer) widget, "clicked", G_CALLBACK(button_clicked), NULL);
        ...
        g_signal_connect((gpointer) widget, "clicked", G_CALLBACK(button_clicked), (gpointer) TRUE);
        ...
        for (i = 0; i < len; i++) {
          ...
          i = end_index;
          if (v->type == V_STR && v->v.p.ptr && strcmp((const char *)v->v.p.ptr, s->str) == 0) {
    ...
    if (widget == 0) {
      ...
      g_signal_connect((gpointer) widget, "clicked",
                       G_CALLBACK(button_clicked), (gpointer) (mode == m_unset ? TRUE : FALSE));
    }
    ...
    if (v->type == V_STR && v->v.p.ptr && strcmp((const char *)v->v.p.ptr, caption) == 0) {
...
void cmd_text() {
  ...
  if (arg.type == V_STR) {
    GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(output.main_view->parent),
