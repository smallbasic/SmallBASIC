...
gboolean key_press_event(GtkWidget *widget, GdkEventKey *event, keymap_data *data);
...
int osd_events(int wait_flag) {
  if (wait_flag || (output.pen_down == 0 && output.pen_mode == PEN_ON && 
...
int osd_getpen(int code) {
  ...
  case 0:                      // return true if there is a waiting pen event(up/down)
    if (output.pen_down != 0) {
      gtk_main_iteration_do(FALSE);
      gdk_window_get_pointer(output.widget->window, &output.pen_down_x, &output.pen_down_y, &mask);
      return 1;
    }
...
void osd_setpixel(int x, int y) {
  ...
  gdk_draw_image(output.pixmap, output.gc, image, 0, 0, x, y, 1, 1);
...
void dev_image(int handle, int index, int x, int y, int sx, int sy, int w, int h) {
  ...
  if (filep->open_flags == DEV_FILE_INPUT) {
    ...
    if (pixbuf) {
      gdk_draw_pixbuf(output.pixmap, output.gc, pixbuf, sx, sy, x, y, w, h, GDK_RGB_DITHER_NORMAL, 0, 0);
      GdkRectangle rc = { x - 1, y - 1, w + 2, h + 2 };
      gdk_window_invalidate_rect(output.widget->window, &rc, TRUE);
    }
  } else {
    // output screen area image to jpeg
    GdkPixbuf *pixbuf = gdk_pixbuf_get_from_drawable(NULL, output.pixmap, NULL,
                                                     x, y, 0, 0, sx, sy);
    if (pixbuf) {
      GError *error = 0;
      gdk_pixbuf_save(pixbuf, filep->name, "jpeg", &error, "quality", "100", NULL);
...
gboolean focus_event(GtkWidget *widget, GdkEventFocus *event, GtkIMContext *imctx) {
#ifdef USE_HILDON
  if (event->in) {
...
gboolean im_context_commit(GtkIMContext *ctx, const gchar *str, keymap_data *data) {
  if (str && str[0]) {
...
void handle_key(int index, int def_key, int keyval, keymap_data *data) {
  if (keymap[index]) {
    ...
    gtk_box_pack_start_defaults(GTK_BOX(GTK_DIALOG(data->dialog)->vbox), data->label);
    g_signal_connect(G_OBJECT(data->dialog), "key_press_event", G_CALLBACK(key_press_event), data);

    gtk_widget_show_all(data->dialog);

    data->imctx = gtk_im_multicontext_new();
    g_signal_connect(G_OBJECT(data->imctx), "commit", G_CALLBACK(im_context_commit), data);
    gtk_im_context_set_client_window(data->imctx, GTK_WIDGET(data->dialog)->window);
    g_signal_connect(G_OBJECT(data->dialog), "focus_in_event", G_CALLBACK(focus_event), data->imctx);
    g_signal_connect(G_OBJECT(data->dialog), "focus_out_event", G_CALLBACK(focus_event), data->imctx);
...
char *dev_gets(char *dest, int size) {
  ...
  gtk_layout_put(GTK_LAYOUT(output.widget), entry, output.cur_x - 1, output.cur_y - 1);
  ...
  g_signal_connect(G_OBJECT(entry), "key_press_event", G_CALLBACK(key_press_event), NULL);
  g_signal_connect(G_OBJECT(entry), "changed", G_CALLBACK(input_changed), entry);
...
void on_realize(GtkWidget *widget, gpointer user_data) {
  ...
  if (output.pixmap) {
    ...
    gdk_draw_drawable(pixmap, output.gc, output.pixmap, 0, 0, 0, 0,     // src/dest x/y
                      old_w, old_h);
    g_object_unref(output.pixmap);
    output.pixmap = pixmap;
  } else {
    // create a new pixmap
    output.pixmap = gdk_pixmap_new(widget->window, output.width, output.height, -1);
    osd_cls();
  }
}
...
void on_size_allocate(GtkWidget *widget, GdkRectangle *allocation, gpointer user_data) {
  if (output.width != allocation->width || output.height != allocation->height) {
...
gboolean expose_event(GtkWidget *widget, GdkEventExpose *event) {
  if (output.pixmap) {
    gdk_draw_drawable(GTK_LAYOUT(widget)->bin_window, output.gc, output.pixmap, event->area.x, event->area.y,   // src 
                                                                                                                // 
...
gboolean drawing_area_init(GtkWidget *main_window) {
  GtkWidget *drawing_area = g_object_get_data(G_OBJECT(main_window), "drawing_area");

  output.main_view = main_window;

  // connect signals
  g_signal_connect(G_OBJECT(drawing_area), "size_allocate", G_CALLBACK(on_size_allocate), NULL);
  g_signal_connect(G_OBJECT(drawing_area), "realize", G_CALLBACK(on_realize), NULL);
