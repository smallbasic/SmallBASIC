...
void speak_string(const char *s, int len) {
  ...
  osso_rpc_run(output.osso, "com.nokia.flite", "/com/nokia/flite", 
               "com.nokia.flite", "flite_tts", 
               &retval, DBUS_TYPE_UINT32,   // id for the order
               getpid(), DBUS_TYPE_STRING,  // string to be read
               text->str, DBUS_TYPE_UINT32, // priority. From 0 to 20.
               20, DBUS_TYPE_DOUBLE,    // speed of speeching. From 0.1 to 3
               speed, DBUS_TYPE_DOUBLE, // adjust pitch of voice. From -2 to 8
               pitch, DBUS_TYPE_STRING, // name of the voice (not implemented)
               "cmu_us_kal", DBUS_TYPE_INVALID);
...
void new_line() {
  ...
  if (output.cur_y + font_height + font_height >= h) {
    ...
    gint w = output.width;
    gdk_draw_drawable(output.pixmap, output.gc, output.pixmap, 0, font_height,  // src x,y
                      0, 0,     // dest x,y
                      w, h - font_height);
    // erase bottom line
    gdk_gc_set_rgb_fg_color(output.gc, &output.bg);
    gdk_draw_rectangle(output.pixmap, output.gc, TRUE, 0, h - font_height, w, font_height);
    osd_refresh();
  } else {
  ...
  while (*p) {
    ...
    default:
      ...
      gdk_draw_rectangle(output.pixmap, output.gc, TRUE,
                         output.cur_x, output.cur_y, cx, output.ascent + output.descent);

      pango_layout_set_text(output.layout, (const char *)p, num_chars);
      gdk_gc_set_rgb_fg_color(output.gc, &output.fg);
      gdk_draw_layout(output.pixmap, output.gc, output.cur_x, output.cur_y, output.layout);

      if (output.underline) {
        gdk_draw_line(output.pixmap, output.gc,
                      output.cur_x,
                      output.cur_y + output.ascent + 1, output.cur_x + cx, output.cur_y + output.ascent + 1);
      }
      output.cur_x += cx;
