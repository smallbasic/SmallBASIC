...
{
#if defined(_UnixOS)
	if ( isatty (STDIN_FILENO)) 	tcgetattr (STDIN_FILENO, &saved_stdin_attributes);
	if ( isatty (STDOUT_FILENO))	tcgetattr (STDOUT_FILENO, &saved_stdout_attributes);
	if ( isatty (STDERR_FILENO))	tcgetattr (STDERR_FILENO, &saved_stderr_attributes);
#endif
}
...
{
#if defined(_UnixOS)
	if ( isatty (STDIN_FILENO)) 	tcsetattr (STDIN_FILENO, TCSANOW, &saved_stdin_attributes);
	if ( isatty (STDOUT_FILENO))	tcsetattr (STDOUT_FILENO, TCSANOW, &saved_stdout_attributes);
	if ( isatty (STDERR_FILENO))	tcsetattr (STDERR_FILENO, TCSANOW, &saved_stderr_attributes);
#endif
}
...
void	term_drawrect(int x1,int y1, int x2, int y2, int fill)
{
	...
	if	( !fill )	{
		...
		term_setxy(x1, y1);	term_print("+");	term_update_color(x1, y1, '+', dev_fgcolor, dev_bgcolor);
		term_setxy(x1, y2);	term_print("+");	term_update_color(x1, y2, '+', dev_fgcolor, dev_bgcolor);
		term_setxy(x2, y1);	term_print("+");	term_update_color(x2, y1, '+', dev_fgcolor, dev_bgcolor);
		term_setxy(x2, y2);	term_print("+");	term_update_color(x2, y2, '+', dev_fgcolor, dev_bgcolor);
		}
	else	{
