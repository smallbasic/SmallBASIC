...
void	cmd_lins()
{
	...
	// get 'index'
	idx = par_getint();	if	( prog_error )	return;
	...
	do	{
		...
		if	( ladd )	// append
			...
			for ( i = var_p->v.a.size-1; i > idx; i -- )	
				// A(i) = A(i-1)
				v_set((var_t*) (var_p->v.a.ptr + (sizeof(var_t) * i)), (var_t*) (var_p->v.a.ptr + (sizeof(var_t) * (i-1))));
...
void	cmd_ldel()
{
	...
	if	( code_peek() == kwTYPE_SEP )	{
		par_getcomma();		if	( prog_error )	return;
		count = par_getint();	if	( prog_error )	return;
			...
			while ( cur_par_idx < pcount && !prog_error )	{
				...
				if	( input_is_finished )	{
					...
					if	( par->flags & PAR_BYVAL )	{
						...
						// check if user had specify a delimiter (next parameter is NOT a variable)
						next_is_const = 0;
...
void	cmd_udp(int cmd)
{
	...
	param.x.vcall.pcount = pcount;			// number of parameters which passed on (the number of parameter-nodes in the stack)
...
void	cmd_param()
{
	...
	if	( pcount != ncall.x.vcall.pcount )	{		// the number of the parameters that are required by this procedure/function
	...
	if	( pcount )	{								// get parameters
		...
		for ( i = 0; i < pcount; i ++ )	{			// check parameters one-by-one
			...
			if	( (vattr & 0x80) == 0 )	{			// UDP requires a 'by value' parameter; any value is good
				node.type = kwTYPE_CRVAR;
				node.x.vdvar.vid = vid;
				node.x.vdvar.vptr = tvar[vid];
...
void	cmd_udpret()
{
	...
	code_pop(&node);
	while ( (node.type != kwPROC) && (node.type != kwFUNC) )	{ 	// pop from stack until caller's node found
		...
		if	( node.type == kwTYPE_CRVAR )	{ 					// local variable - cleanup
			...
			tvar[node.x.vdvar.vid] = node.x.vdvar.vptr;			// restore ptr (replace to pre-call variable)
			}
...
//		}
	...
	if	(  code_peek() != kwIN )	{
		...
		if	( code_isvar() )	{
			...
			new_var = v_new();
			eval(new_var);
			if	( prog_error )	{	tmp_free(new_var);	return; }
...
void	cmd_delay()
{
	...
	ms = par_getint();		if	( prog_error )	return;
	dev_delay(ms);
}
...
void	cmd_locate()
{
	...
	y = par_getint()-1;		if	( prog_error )	return;
	    par_getcomma();		if	( prog_error )	return;
	x = par_getint()-1;		if	( prog_error )	return;
	#if defined(_PalmOS)
...
void	cmd_color()
{
	...
	fg = par_getint();		if	( prog_error )	return;
	if	( code_peek() == kwTYPE_SEP )	{
	    par_getcomma();			if	( prog_error )	return;
		bg = par_getint();		if	( prog_error )	return;
		}
...
void	cmd_wjoin()
{
	...
	v_init(&del);
	var_p = par_getvarray();	if	( prog_error )		return;
	par_getcomma();		if	( prog_error )	return; 
	par_getstr(&del);	if	( prog_error )	return; 
	par_getcomma();		if	( prog_error )	{ v_free(&del); return; }
	if	( !code_isvar() )	{
		err_syntax();
		v_free(&del);
		return;
...
void	cmd_datedmy()
{
	...
	// byref pars
 	par_getcomma();			if	( prog_error )	return;
	vd = par_getvar_ptr();	if	( prog_error )	return;
 	par_getcomma();			if	( prog_error )	return;
	vm = par_getvar_ptr();	if	( prog_error )	return;
 	par_getcomma();			if	( prog_error )	return;
	vy = par_getvar_ptr();	if	( prog_error )	return;

	v_free(vd);
	v_free(vm);
	v_free(vy);

	vd->type = V_INT;	vd->v.i = d;
	vm->type = V_INT;	vm->v.i = m;
	vy->type = V_INT;	vy->v.i = y;
}
...
void	cmd_timehms()
{
	...
	// byref pars
 	par_getcomma();			if	( prog_error )	return;
	vh = par_getvar_ptr();	if	( prog_error )	return;
 	par_getcomma();			if	( prog_error )	return;
	vm = par_getvar_ptr();	if	( prog_error )	return;
 	par_getcomma();			if	( prog_error )	return;
	vs = par_getvar_ptr();	if	( prog_error )	return;

	v_free(vh);
	v_free(vm);
	v_free(vs);

	vh->type = V_INT;	vh->v.i = h;
	vm->type = V_INT;	vm->v.i = m;
	vs->type = V_INT;	vs->v.i = s;
}
...
void	cmd_search()
{
	...
	par_getcomma();		if ( prog_error )	{ v_free(&vkey); return; }
	if	( code_isvar() )	{
		rv_p = code_getvarptr();
		v_free(rv_p);
		}
...
void	cmd_swap(void)
{
	...
	if	( code_isvar() )	va = code_getvarptr();	else	{ err_typemismatch(); return; }
	par_getcomma();											{ if ( prog_error )	  return; }
	if	( code_isvar() )	vb = code_getvarptr();	else	{ err_typemismatch(); return; }
...
void	cmd_poke(void)
{
	...
	addr = par_getint();	if	( prog_error )	return;	
	par_getcomma();			if	( prog_error )	return;
	val = par_getint();		if	( prog_error )	return;	
...
void	cmd_bcopy(void)
{
	...
	par_getcomma();			if	( prog_error )	return;
	size = par_getint();		if	( prog_error )	return;	
...
void cmd_image() {
    ...
    if (code_peek() == kwTYPE_SEP) {
        par_getcomma();       if (prog_error) return;
        sx = par_getint();    if (prog_error) return;
        par_getcomma();       if (prog_error) return;
        sy = par_getint();    if (prog_error) return;
        if (code_peek() == kwTYPE_SEP) {
            par_getcomma();   if (prog_error) return;
            iw = par_getint(); if (prog_error) return;
            par_getcomma();   if (prog_error) return;
            ih = par_getint(); if (prog_error) return;
        }
    }
