...
void	osd_write(const char *str)
{
	...
	while ( *p )	{
		...
		default:
			...
			FBputchar(fdev, curcol, curline*FONT_HEIGHT, vga16[text_fgcolor], vga16[text_bgcolor], *p);
			curcol += cx;
			};
...
int		ufb_events(int wait_flag)
{
	...
	if	( wait_flag == 0 )	{
		...
		case	FBKeyEvent:
			FBgetevent(fdev, &ev);
			if	( (ev.key.ascii >= 32) || (ev.key.ascii & 0x80) || (ev.key.ascii == 9) || (ev.key.ascii == 13) || (ev.key.ascii == 10)  || (ev.key.ascii == 8) )
