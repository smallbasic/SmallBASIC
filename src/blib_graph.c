...
void    cmd_view()
{
    ...
    if  ( code_peek() != kwTYPE_EOC && code_peek() != kwTYPE_LINE ) {
        p1 = par_getipt();      if  ( prog_error )  return;
        par_getcomma();         if  ( prog_error )  return;
        p2 = par_getipt();      if  ( prog_error )  return;
        if  ( code_peek() == kwTYPE_SEP )   {
            par_getcomma();         if  ( prog_error )  return;
            color = par_getint();   if  ( prog_error )  return;
            if  ( code_peek() == kwTYPE_SEP )   {
                par_getcomma();         if  ( prog_error )  return;
                bcolor = par_getint();  if  ( prog_error )  return;
                }
            }
...
void    cmd_rect()
{
    ...
    if  ( code_peek() != kwCOLOR && code_peek() != kwFILLED && code_peek() != kwTYPE_EOC && code_peek() != kwTYPE_LINE )    {
        /* [STEP] x, y [[,]STEP] ---> x2, y2 */
        p2 = par_getipt();
        if  ( step )
            ( p2.x += p1.x, p2.y += p1.y );
...
void    cmd_circle()
{
    ...
    // xc,yc
    pt = par_getipt();  if ( prog_error )   return;
    // r
    par_getcomma();     if ( prog_error )   return;
    r = par_getint();   if ( prog_error )   return;
    if  ( step )
        ( pt.x += gra_x, pt.y += gra_y );
    ...
    if  ( code_peek() == kwTYPE_SEP )   {
        par_getcomma();             if ( prog_error )   return;
        aspect = par_getnum();      if ( prog_error )   return;
...
void    cmd_arc()
{
    ...
    // a.st.
    par_getcomma();     if ( prog_error )   return;
    as = par_getnum();  if ( prog_error )   return;

    // a.end
    par_getcomma();     if ( prog_error )   return;
    ae = par_getnum();  if ( prog_error )   return;
...
void    cmd_paint()
{
    ...
    if  ( code_peek() == kwTYPE_SEP )   {
        par_getcomma();         if ( prog_error )   return;
        fc = par_getint();      if ( prog_error )   return;
        }

    // bordercolor
    if  ( code_peek() == kwTYPE_SEP )   {
        par_getcomma();             if ( prog_error )   return;
        bc = par_getint();      if ( prog_error )   return;
        }
...
void    cmd_draw()
{
    ...
    par_getstr(&var);   if ( prog_error )   return;
    p = (char *) var.v.p.ptr;
    while ( *p )    {
        ...
        case 'M': case 'm': // move to x, y
            ...
            if  ( *p != ',' )   {
                ...
                // Haraszti -- next pointer forward is an error because draw_getval contain p++ too!!!
                //p ++;
                }
            p = draw_getval(p, &y);
        ...
        case 'C': case 'c': // color
            ...
            dev_setcolor(x);
            continue;
        // Haraszti -- next case filter out the spaces or tabs and semicolons (GWBASIC compatibility)
        case ' ': case '\t': case ';':
            p++;
            continue;
        default:
...
*/
void    chart_draw(int x1, int y1, int x2, int y2, double *vals, int count, double *xvals, int xcount, int chart, int marks)
{
    int     *pts;
    int     rx1, dx, dy, i;
    ...
    case    2:  // bar chart
        ...
        if  ( os_color_depth > 2 )  
            dev_setcolor(color);
        dev_rect(
            pts[(count-1)*2], pts[(count-1)*2+1], 
            pts[(count-1)*2]+lx-1, y2, 
            1);
...
void    cmd_chart()
{
    ...
    if  ( code_peek() == kwTYPE_SEP )   {
        par_getcomma();             if  ( prog_error )      return;
        marks = par_getint();       if  ( prog_error )      return;

        // optional x1,y1,x2,y2
        if  ( code_peek() == kwTYPE_SEP )   {
            par_getcomma();         if  ( prog_error )      return;
            x1 = par_getint();      if  ( prog_error )      return;
            par_getcomma();         if  ( prog_error )      return;
            y1 = par_getint();      if  ( prog_error )      return;
            par_getcomma();         if  ( prog_error )      return;
            x2 = par_getint();      if  ( prog_error )      return;
            par_getcomma();         if  ( prog_error )      return;
            y2 = par_getint();      if  ( prog_error )      return;
            }
        }
...
void    cmd_m3rotate()
{
    ...
    m = par_getm3();        if  ( prog_error )  return;
    par_getcomma();         if  ( prog_error )  return;
    angle = par_getnum();   if  ( prog_error )  return;
    if  ( code_peek() == kwTYPE_SEP )   {
        par_getcomma();         if  ( prog_error )  return;
        x = par_getnum();       if  ( prog_error )  return;
        par_getcomma();         if  ( prog_error )  return;
        y = par_getnum();       if  ( prog_error )  return;
        }
...
void    cmd_m3scale()
{
    ...
    y = par_getnum();       if  ( prog_error )  return;
    par_getcomma();         if  ( prog_error )  return;
    fx = par_getnum();      if  ( prog_error )  return;
    par_getcomma();         if  ( prog_error )  return;
    fy = par_getnum();      if  ( prog_error )  return;
...
void    cmd_m3apply()
{
    ...
    m = par_getm3();        if  ( prog_error )  return;
    par_getcomma();         if  ( prog_error )  return;
    p = par_getvarray();    if  ( prog_error )  return;
    count = p->v.a.size;
...
//
//   Υπολογίζει το σημείο τομής (R) των ευθύγραμμων τμημάτων
//   Α-Β καί C-D.
//
//   Επιστρέφει:
//   R       = Το σημείο τομής
//   (return code) type = Είδος τομής.
//           0 = δεν υπάρχει τομή (επιστρέφει εξωτερική τομή NDC 04/01/95).
//           1 = υπάρχει μία τομή.
//           2 = παράλληλα.
//           3 = παράλληλα με πολλές τομές (συμπίπτουν)...
//           4 = Το σημείο τομής είναι άκρη ευθείας.
//
void    cmd_intersect()
{
    double  a, b, c, s;
    ...
    // parameters
    A = par_getpt();            if  ( prog_error )  return;
    par_getcomma();             if  ( prog_error )  return;
    B = par_getpt();            if  ( prog_error )  return;
    par_getcomma();             if  ( prog_error )  return;
    C = par_getpt();            if  ( prog_error )  return;
    par_getcomma();             if  ( prog_error )  return;
    D = par_getpt();            if  ( prog_error )  return;
    par_getcomma();             if  ( prog_error )  return;

    type = par_getvar_ptr();    if  ( prog_error )  return;
    par_getcomma();             if  ( prog_error )  return;

    vrx = par_getvar_ptr();     if  ( prog_error )  return;
    if  ( code_peek() == kwTYPE_SEP )   {
        par_getcomma();             if  ( prog_error )  return;
        vry = par_getvar_ptr();     if  ( prog_error )  return;
        }
    else
        style = 1;
    ...
    if ( a == 0.0 ) 
        type->v.i = ( b == 0.0 ) ? 3 : 2;  // Αν b=0, συμπίπτουν αλλιώς παραλληλα
    else    {
        if ( a > 0.0 )  {
    ...
    if ( type->v.i == 1 || type->v.i == 0 ) {
        ...
        if (b == a || c == a || c == 0.0) 
            if (type->v.i == 1) type->v.i = 4;  // Special case
...
void    cmd_polyext()
{
    ...
    // initialize vars
    v_free(xmin);   xmin->type = V_NUM; 
    v_free(xmax);   xmax->type = V_NUM;
    v_free(ymin);   ymin->type = V_NUM; 
    v_free(ymax);   ymax->type = V_NUM;
