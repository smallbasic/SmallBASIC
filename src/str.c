...
*	strings!
*
*	Nicholas Christopoulos, 5/5/2000
*
...
char	*q_strstr(const char *s1, const char *s2, const char *pairs)
{
	...
	while ( *p )	{
		...
		if	( *p == wait_q )	{		// i am waiting that. level down
		...
		else if	( (z=strchr(pairs, *p)) != NULL )	{	// character is a delimiter; level up
			open_q = ((z-pairs)+1) % 2;	// true, if its a 'begin' delimiter
			...
			if	( wait_q && open_q )	{
				...
				;							// do nothing, I am waitting something else
...
//	return (strchr("_ÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÓÔÕÖ×ØÙáâãäåæçèéêëìíîïðñóôõö÷øùÜüÞÝýþßúû", ch) != NULL); // Greek
...
char	*get_numexpr(char *text, char *dest, int *type, long *lv, double *dv)
{
	...
	*dv = 0.0;
	...
	// sign
	if  ((*p == '-' || *p == '+') && strchr("0123456789.", *(p+1)) && *(p+1) != '\0')	{ 
		if	( *p == '-' )
			sign = -1;
	...
	if	( (*p == '&')
		  || 
		  (*p == '0' && (*(p+1) != '\0' && strchr("HXBO", to_upper(*(p+1))) != NULL)
		  ))	{
		...
		if ( (*p == 'E' || *p == 'e') && (*type == 0) )	{
			...
			*d ++ = *p ++;			// E
			...
			if	( *p == '+' || *p == '-' || is_digit(*p) || *p == '.' )	{
				...
				if	( *p == '+' || *p == '-' )	{
					*d ++ = *p ++;
					if	( strchr("+-*/\\^", *p) != 0 )	{	// stupid E format (1E--9 || 1E++9)
						stupid_e_fmt = 1;
						eop = *p;
						*d ++ = *p ++;
						if	( *p == '+' || *p == '-' )
...
/*
						if	( *p == 'E' || *p == 'e' )	{
							...
							switch ( r_type )	{
							case	1:	power = r_lv;	break;
							case	2:	power = r_dv;	break;
							default:	// error
								*type = r_type;
								}
							}
...
char	*discldup(const char *source, const char *pairs, const char *ignpairs)
{
	...
	while ( *p )	{
		...
		// primary pairs
		else if	( *p == wait_p && wait_q == 0 )	{	// primary pair - level down
			level_p --;
			if	( level_p <= 0 )	{
				// store and exit
				record = 0;
