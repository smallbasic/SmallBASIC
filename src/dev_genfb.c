...
int		gfb_init(int width, int height, int bpp)
{
	...
	// setting up vram
	dev_vpage = tmp_alloc(dev_video_size+(dev_linelen*17));	// the additional size is added for not checking drawchar pixels
...
static void	direct_hline(int x, int x2, int y)
{
	...
	if	( x < 0 )			x = 0;
	if	( x2 < 0 )			return;
	if	( x2 >= dev_width )	x2 = dev_width - 1;
	if	( y < 0 )			return;
	if	( y >= dev_height )	return;
